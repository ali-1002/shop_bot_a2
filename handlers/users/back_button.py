...
@dp.callback_query_handler(text=["back_tv", "back_audio", "back_photo_video",
                                 "back_noutbook", "back_monitor", "back_monoblok",
                                 "back_smartfon", "back_sm_gadjets", "back_washing_machine", "back_air_conditioner"])
...
async def back_product_list(call: CallbackQuery):
    product_id = int(call.message.reply_markup.inline_keyboard[2][0].callback_data.split('back_product_list_')[1])
    ...
    btn = main_menu(product_list, product_subcategory_code=subcategory_code, product=True)
    await call.message.edit_text(
        subcategory_name, reply_markup=btn
    )
